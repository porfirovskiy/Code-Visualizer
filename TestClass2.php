...
class Seo_model extends CI_Model
{
    ...
    public function get_search_sql($type)
    {
        if ($type == self::FORWARD_PREFIX) {
            return "SELECT se.word, CONCAT('/', 'english', '-', 
                        LOWER(REPLACE(REPLACE(REPLACE(l.name, '(', '<'), ')', '>'), ' ', '_')),
                        '/', LOWER(REPLACE(se.word, '\"', '')), '-', st.id) AS url
            ...
            return 'SELECT st.translation AS word, CONCAT("/", 
                        LOWER(REPLACE(REPLACE(REPLACE(l.name, "(", "<"), ")", ">"), " ", "_")),
                        "-", "english", "/", LOWER(REPLACE(st.translation, "\'", "\\"")), "-", st.id) AS url
